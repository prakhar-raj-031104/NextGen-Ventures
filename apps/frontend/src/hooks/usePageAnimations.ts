...
export const usePageAnimations = () => {
  ...
  useLayoutEffect(() => {
    ...
    const context = gsap.context(() => {
      ...
      const sideOffset = compactViewport ? 12 : 72;
      ...
      gsap.utils.toArray<HTMLElement>("[data-reveal]").forEach((element, index) => {
        ...
        gsap.fromTo(
          element,
          { x: direction * sideOffset, y: 20, autoAlpha: 0, filter: "blur(10px)" },
          {
            x: 0,
            y: 0,
            autoAlpha: 1,
      ...
      // --- Staggered grid/group animations for [data-stagger] containers ---
      ...
      // --- Stat counter animations for [data-count] elements ---
      ...
      gsap.utils.toArray<HTMLElement>(".process-section, .closing-cta, .service-fit").forEach(
        (element) => {
          gsap.fromTo(
            element,
            { "--border-progress": "0%" },
